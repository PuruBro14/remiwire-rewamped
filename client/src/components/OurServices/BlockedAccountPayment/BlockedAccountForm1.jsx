...
export default function BlockedAccountForm1({ setFormStep,
  ...
  const handleSubmit = (e) => {
    ...
    if (!blockeAccountForms.transferFromState) {
      newErrors.transferFromState = "State is required";
    }

    if (!blockeAccountForms.transferFromCity) {
      newErrors.transferFromCity = "City is required";
    }

    if (!blockeAccountForms.receivingAmountInEuro) {
      newErrors.receivingAmountInEuro = "Amount in Euro is required";
    }

    if (!blockeAccountForms.receivingAmountInINR) {
  ...
  const getCurrentRateINRtoEURO = async () => {
    try {
      const response = await axios.get(
        "https://v6.exchangerate-api.com/v6/1902e21487d17680cb9fc088/latest/EUR"
      );
      const { INR } = response.data.conversion_rates;

      dispatch(setformValue({ oneEurotoINR: INR }));
      const inrValue = 11208 * blockeAccountForms.oneEurotoINR;
  ...
  }, [blockeAccountForms.oneEurotoINR]);
        ...
        <form className="max-w-xl mx-auto" onSubmit={handleSubmit}>
          ...
          <div>
            <div>
              ...
              <input
                type="text"
                readOnly={true}
                value={blockeAccountForms.transferToCountry}
          ...
          <div className="mt-5">
            <div className="w-full mb-5 group">
              ...
              <input
                readOnly={true}
                value={blockeAccountForms.purposeOfTransfer}
                className="w-full border border-gray-300 rounded-[5px] text-gray p-1"
              />
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 md:gap-6">
            <div className="relative z-0 w-full mb-5 group">
              <input
                type="text"
                readOnly={true}
                value={blockeAccountForms.receivingCurrency}
                name="courseDetails"
                id="course_details"
            ...
            <div className="relative z-0 w-full mb-5 group">
              <input
                type="text"
                readOnly={true}
                value={blockeAccountForms.sendingCurrencyIn}
                className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                placeholder=" "
              />
            </div>
            ...
            <div className="relative z-0 w-full mb-5 group">
              <div className="relative">
                <input
                  ...
                  value={blockeAccountForms.receivingAmountInEuro}
                  onChange={(e) => {
                    const inrValue =
                      e.target.value * blockeAccountForms.oneEurotoINR;
                    handleInputChange("receivingAmountInINR", inrValue);
            ...
            <div className="relative z-0 w-full mb-5 group">
              <div className="relative">
                <input
                  ...
                  value={blockeAccountForms.receivingAmountInINR}
                  onChange={(e) => {
                    const inputValue = parseFloat(e.target.value);
                    const eurValue =
                      inputValue / blockeAccountForms.oneEurotoINR;
